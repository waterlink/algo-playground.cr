...
describe PathFinding do
  ...
  describe "#find_path" do
    ...
    it "finds any path" do
      ...
      PathFinding.find_path({0 => [1], 1 => [2]}, 1, 3)
        .should eq({true, [1, 2]})
      ...
      PathFinding.find_path({0 => [5, 1, 6], 1 => [3, 2, 4]}, 0, 2)
        .should eq({true, [0, 1, 2]})

      PathFinding.find_path({0 => [5, 6, 1], 1 => [3, 2, 4]}, 0, 2)
        .should eq({true, [0, 1, 2]})

      PathFinding.find_path({0 => [1, 2, 3], 2 => [3, 4, 5], 5 => [7, 8, 11]}, 0, 11)
        .should eq({true, [0, 2, 5, 11]})

      PathFinding.find_path({0 => [1, 2, 3], 2 => [3, 4, 5], 5 => [7, 0, 8, 11]}, 0, 11)
        .should eq({true, [0, 2, 5, 11]})
    end
  end
end
